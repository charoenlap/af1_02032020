...
function postConfirm()
{
    ...
    $.ajax({
        ...
        success: function(result) {
            // console.log(result);
            // console.log($("input[name='customer_id']").val());
            var cus_id = $("input[name='customer_id']").val();
            if (result.status == 'success') {
                var info = [];
                $('.mw-checkbox-point:checked').each(function () {
                    var ele = $(this);
                    var cr = ele.parents('tr').find("input[type='text']").val();
                    var val = ele.parents('tr').find("input[type='number']").val();
                    var connote = ele.parents('tr').find(".connote").val();
                    var temp_arr = [ele.attr('data-id'),cr,val,connote];
                    // console.log('cr'+cr+': val'+val);
                    info.push(temp_arr);
                });
                $.ajax({
                    url: 'http://af1express.com/api/ajax/add_connotes.php',
                    type: 'GET',
                    dataType: 'html',
                    data: {
                        info: info,
...
function genFormConfirm()
{
    ...
    form += '<div class="col-sm-12">';
        ...
        form += '<table class="table table-bordered">';
            ...
            form += '<th width="40%">Customer Ref.</th>';
            form += '<th width="30%">จำนวนเงิน</th>';
            form += '<th width="10%" class="mw-center">หน่วย</th></tr>';
            form += '</thead>'
            form += '<tbody>';
            var i=0;
            $('#table-listcustomer tbody tr .mw-checkbox-point:checked').each(function( index ) {
                // if($(this).checked){
                    var cusref = $(this).parents('tr').find('.form-control.mw-mg-0').val();
                    var price = $(this).parents('tr').find('input[type="number"]').val();

                    form += '<tr>';
                    form += '<td class="mw-center">'+(i+1)+' '+'</td>';
                    
                    form += '<td>'+'<p class="mw-mg-0"><small>'+$(this).attr('data-address')+' '+$(this).attr('data-district')+' '+$(this).attr('data-province')+' '+$(this).attr('data-postcode')+'</small></p></td>'+'</td>';
                    form += '<td class="mw-center">'+cusref+'</td>';
                    form += '<td>'+price+'</td>';
